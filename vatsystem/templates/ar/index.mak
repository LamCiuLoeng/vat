...
<script type="text/javascript">
    $(function() {
        ...
        $(".ajaxSearchField").each(function(){
        	...
        	if(type == 'customer_name' || type == 'customer_short_name')type='cust_name'
        	if(type == 'customer_type')type='cust_type'
...
<div class='submenu' id="function-menu">
    <ul>
        ...
        <li class='li-center'> <a href="/ar/index"><img src="/images/images/VAT-ERP_g.jpg"></a></li>
        <li class='li-end'></li>
    </ul> 
</div>
</%def>
<div class="ui-layout-west">
    ...
    <div id="accordion" class="search none">
        <h3><a>Search SI</a></h3>
        <div>
            <form action="/ar" method="get" id="_form0">
                <input type="hidden" name="type" value="${const.SEARCH_TYPE_SI}" />
                %if kw.get('type','')==const.SEARCH_TYPE_SI:
                ${si_search_form(value=kw)|n}
                %else:
                ${si_search_form()|n} 
                %endif
                <br/>
        ...
        <h3><a>Search SO</a></h3>
        <div>
            <form action="/ar" method="get" id="_form1">
                <input type="hidden" name="type" value="${const.SEARCH_TYPE_SO}" />
                %if kw.get('type','')==const.SEARCH_TYPE_SO:
                ${so_search_form(value=kw)|n}
                %else:
                ${so_search_form()|n}
                %endif
                <br/>
        ...
        <div>
            <form action="/ar" method="get" id="_form2">
            	<a onclick="viewAll('/ar?type=${const.SEARCH_TYPE_MSIMSO}&date_from=${last_month()[0]}&date_to=${last_month()[1]}')"  href="javascript:void(0)">Last Month</a><br/>
        ...
        <div>
            <form action="/ar" method="get" id="_form3">
                <a href="javascript:void(0)" onclick="viewAll('/ar?type=${const.SEARCH_TYPE_MCN}&date_from=${last_month()[0]}&date_to=${last_month()[1]}')" >Last Month</a><br/>
        ...
        <h3><a>Search Group SI</a></h3>
        <div>
            <form action="/ar" method="get" id="_form4">
                <a href="javascript:void(0)" onclick="viewAll('/ar?type=${const.SEARCH_TYPE_GROUP_SI}&date_from=${last_month()[0]}&date_to=${last_month()[1]}')" >Last Month</a><br/>
                <input type="hidden" name="type" value="${const.SEARCH_TYPE_GROUP_SI}" />
                %if kw.get('type','')==const.SEARCH_TYPE_GROUP_SI:
                ${si_group_search_form(value=kw)|n}
                %else:
                ${si_group_search_form()|n}
                %endif
                <br/>
                <input type="button" class="btn" value="Search" onclick="toCheckSearch(4)"/>
                <input type="button" class="btn" value="Reset" onclick="toReset(4)"/>
            </form>
        </div>
        <h3><a>Search Group SO</a></h3>
        <div>
            <form action="/ar" method="get" id="_form5">
                <a href="javascript:void(0)" onclick="viewAll('/ar?type=${const.SEARCH_TYPE_GROUP_SO}&date_from=${last_month()[0]}&date_to=${last_month()[1]}')" >Last Month</a><br/>
                <input type="hidden" name="type" value="${const.SEARCH_TYPE_GROUP_SO}" />
                %if kw.get('type','')==const.SEARCH_TYPE_GROUP_SO:
                ${so_group_search_form(value=kw)|n}
                %else:
                ${so_group_search_form()|n}
                %endif
                <br/>
                <input type="button" class="btn" value="Search" onclick="toCheckSearch(5)"/>
...
<div class="ui-layout-center ui-layout-pane ui-layout-pane-center" style="position: absolute; margin: 0px; left: 166px; right: 0px; top: 144px; bottom: 0px; height: 243px; width: 1272px; z-index: 1; visibility: visible; display: block;">
    <div id="tabs" class="ui-tabs ui-widget ui-widget-content ui-corner-all" >
        ...
        <%include file="vatsystem.templates.ar.list_chead"/>
       	%elif kw.get('type','')==const.SEARCH_TYPE_GROUP_SI:
        <%include file="vatsystem.templates.ar.list_customer"/>
        %elif kw.get('type','')==const.SEARCH_TYPE_GROUP_SO:
        <%include file="vatsystem.templates.ar.list_customer"/>
        %endif
    </div>
</div>
